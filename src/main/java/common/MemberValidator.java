...
차후 supports(), validate() 두 개의 메서드를 오버라이딩해야함
 */
public class MemberValidator implements Validator{
	...
	메서드는 아예 호출되지 않는다. 해당 메서드는 자동으로 호출됨
	 */
	@Override
	...
	/*
	 supports()가 true를 반환하는 경우에만 호출되는 메서드로 실제 폼값에 대한 
	 검증을 진행한다.
	 매개변수1: 폼값을 저장한 커맨드객체
	...
	public void validate(Object obj, Errors errors) {
		...
		if(member_id==null||member_id.trim().isEmpty()) {
			...
			//검증에 실패한 경우 해당 메서드를 통해 검증결과를 반환함
			errors.rejectValue("id", "idError");
		}
		//검증2: 내장메서드를 통한 검증(패스워드 검증)
		/*
		 유효성 검증을 위해 전달되는 객체가 커맨드객체가 아니라면 validate()메서드
		 내부로 진입자체가 불가능하므로, 해당함수에서 pw라는 문자열은 getter() 메서드를
		 통해 DTO에 저장된 패스워드를 얻어오게 된다.
		 */
