...
/**
 ...
 요청명에 매핑된 메서드를 찾아 요청을 전달하고 로직을 수행함
 @Controller
 	:해당 클래스를 컨트롤러로 사용하고 싶을때 클래스명 앞에
 	선언한다. 패키지를 스캔할때 해당 어노테이션이 있는 클래스를 찾아서
 	요청을 전달한다.
 @RequestMapping
 	:요청명을 매핑한다. 요청명은 서블릿과 마찬가지고 컨택스트루트를
...
public class FormController {
	...
	public String loginMember(HttpServletRequest req, Model model) {
		//request내장객체의 getParameter()를 사용한다.
		String id= req.getParameter("id");
		String pw= req.getParameter("pw");
		
		//View로 전달한 데이터를 Model객체에 저장함
	...
	/*
	 ...
	 주의해야 한다. 또한 파라미터의 갯수가 틀려지면 404에러가 발생됨
	 */
